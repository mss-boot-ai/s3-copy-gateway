...
[![Release](https://github.com/mss-boot-ai/s3-copy-gateway/actions/workflows/release.yml/badge.svg)](https://github.com/mss-boot-ai/s3-copy-gateway/actions/workflows/release.yml)
...
Print embedded build metadata with:

```bash
s3-copy-gateway --version
```
...
Pushing a validated `vMAJOR.MINOR.PATCH` tag publishes a GitHub Release with:

- Linux `amd64` and `arm64` binary archives.
- A `SHA256SUMS` verification file.
- A multi-architecture container image for `linux/amd64` and `linux/arm64`.
- Image tags for the exact Git tag and semantic version.
- OCI provenance and SBOM metadata generated by Buildx.

The image for a release can be pulled with its Git tag:

```bash
docker pull ghcr.io/mss-boot-ai/s3-copy-gateway:v0.1.0
```

The generated Release notes include an immutable digest pull command. Public
images are tested without registry credentials on both supported architectures
before the Release is published. See
